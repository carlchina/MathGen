...
const Paper: React.FC<PaperProps> = ({ problems, config, conceptSection, showNeighbors = true }) => {
  ...
  const renderMathEquation = (problem: MathProblem, showAnswers: boolean, showNeighbors: boolean = true) => {
    const text = problem.display;
    
    if (!text.includes('▢')) {
      return (
        <div className={`flex items-center justify-between w-full h-[26px] ${showNeighbors ? 'print:h-[26px]' : 'print:h-[31px]'} border-b border-dashed border-gray-100 pb-0.5 z-0`}>
          <span className={`text-gray-800 font-sans font-semibold text-[14.5px] ${showNeighbors ? 'print:text-[14px]' : 'print:text-[15px]'}`}>{text}</span>
          {showAnswers && (
            <span className={`text-blue-600 font-handwriting font-bold text-[17.5px] ${showNeighbors ? 'print:text-[16px]' : 'print:text-[18px]'} tracking-wide ml-1 select-none transform -rotate-2`}>
              {problem.answer}
            </span>
          )}
        </div>
    ...
    return (
      <div className={`flex items-center w-full h-[26px] ${showNeighbors ? 'print:h-[26px]' : 'print:h-[31px]'} border-b border-dashed border-gray-100 pb-0.5 text-gray-800 font-sans font-semibold text-[14.5px] ${showNeighbors ? 'print:text-[14px]' : 'print:text-[15px]'}`}>
        {parts.map((part, index) => {
          ...
          return (
            <React.Fragment key={index}>
              {renderBox && (
                <span className={`inline-flex items-center justify-center w-[28px] ${showNeighbors ? 'print:w-[26px] print:h-[18px]' : 'print:w-[30px] print:h-[21px]'} h-[20px] border border-gray-400 bg-gray-50/10 rounded mx-1 shrink-0 relative shadow-inner`}>
                  {showAnswers && currentBoxAns ? (
                    <span className={`text-rose-600 font-handwriting font-extrabold text-[14.5px] ${showNeighbors ? 'print:text-[13.5px]' : 'print:text-[15px]'} select-none absolute`}>
                      {currentBoxAns}
                    </span>
                  ) : (
  ...
  return (
    ...
    <div id="worksheet-paper" className="font-serif text-gray-800 box-border">
      ...
      <div className={`grid grid-cols-5 gap-3.5 print:gap-x-3.5 print:gap-y-0 text-sm leading-loose ${showNeighbors ? 'border-b-2 border-gray-900 pb-3 print:pb-3' : 'pb-3 print:pb-0'}`}>
        {columns.map((col, colIndex) => {
          ...
          return (
            // print:gap-y-[2.5px] / print:gap-y-[5.8px] optimizes the vertical space between questions in print mode
            // causing the total height to fill the A4 page beautifully.
            <div key={colIndex} className={`flex flex-col gap-1.5 ${showNeighbors ? 'print:gap-y-[2.5px]' : 'print:gap-y-[5.8px]'}`}>
              <div className="text-center font-bold mb-1 text-gray-800 border-b border-gray-300 pb-1 font-sans text-xs tracking-wide">
                {groupTitle}
              </div>
              {col.map((problem) => (
                <div key={problem.id} className={`whitespace-nowrap h-[26px] ${showNeighbors ? 'print:h-[26px]' : 'print:h-[31px]'} flex items-center`}>
                  {renderMathEquation(problem, !!config.showAnswers, showNeighbors)}
                </div>
              ))}
            </div>
          );
